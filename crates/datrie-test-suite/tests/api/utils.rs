...
use datrie_test_suite::{AlphaChar, AlphaMap, Trie, TrieData};
...
//  *-------------------------*/
pub unsafe fn en_alpha_map_new() -> anyhow::Result<*mut AlphaMap> {
    let en_map = alpha_map_new();
    if en_map.is_null() {
        anyhow::bail!("failed to create alpha_map");
    }
    alpha_map_add_range(en_map, 0x0061, 0x007a);

    Ok(en_map)
}

pub unsafe fn en_trie_new() -> anyhow::Result<*mut Trie> {
    let en_map = en_alpha_map_new()?;

    let en_trie = trie_new(en_map);
    if en_trie.is_null() {
